...
function [mouseexp, recordings] = identifyMouseRecordingDates(serverdir)
   if nargin<1, serverdir = '/Volumes'; end
   expdir      = fullfile( serverdir, 'Schultz_group_data','Crazy Eights','Ann' );
   logbookdir  = fullfile( expdir, 'Digital Logbook', 'GCaMP6', 'By date' );
   gcampdir    = fullfile( expdir, 'GCaMP6 imaging' );
   % months must be capital to separate from minutes
   dateformat  = 'yyyy.mm.dd'; % using matlab's formatting for datestr
   imgformat   = 'yyyymmdd_HH_MM_SS_2P';
   trackformat = 'Track_yyyy-mm-dd-HH-MM-SS';
   logformat   = 'Experiment Data Sheet - yyyy.mm.dd';
   photondir   = '2P';
   trackdirname= 'Neurotar';
   matchtype   = '[0-9a-zA-Z ]+'; % match dir & filenames based on alphanumeric chars (but not '_')
   defaultrate = 30.91; % default scan rate
   ...
   if ~exist( expdir, 'dir' )
      fprintf('Server directory not found (%s), exiting...\n', expdir );
      return;
   end
   ...
   dateregexp  = genRegExpFromFormat( dateformat, matchtype );
      ...
      if ~isempty( regexp( dirname, dateregexp, 'once' ) ) 
         ...
         if ~isempty( imgtimes ) && ~isempty( tracktimes )
            ...
            if ~isempty( finalimgtimes )
               numrec = numrec + 1;
               recordings{numrec}.path      = gcampdir;
               recordings{numrec}.dirname   = dirname;
   ...
   logdates  = dir( logbookdir );
   logregexp = genRegExpFromFormat( logformat, matchtype );
   currnames = []; nummice = 0;
   for li=1:length( logdates )
      % we're looking at filenames here
      filename = logdates(li).name;
      if ~isempty( regexp( filename, logregexp, 'once' ) ) 
         % valid file, so get animal names from worksheets
         [status,sheets] = xlsfinfo( fullfile( logbookdir, filename) );
         % cycle through each worksheet name & extract ones with m<##>
         for si=1:length(sheets)
            if strcmpi( sheets{si}, 'Experiment Details' )
               % scan rate has format <xx.xx> Hz, e.g. 30.91 Hz
               [~,scantxt] = xlsread(fullfile( logbookdir, filename), sheets{si}, 'B11');
               numend      = regexp( scantxt{:}, ' Hz' );
               scanrate    = str2double( scantxt{1}(1:numend-1) );
               if isempty( scanrate )
                  scanrate = defaultrate;
               end
            elseif sheets{si}(1)=='m' && length(sheets{si})==3
               ...
               num = xlsread(fullfile( logbookdir, filename), sheets{si}, 'A15:A100');
               % get hours, minutes, seconds from num
               [h,m,s] = hms(datetime(num,'ConvertFrom','excel')); 
               % get neurotar tracking code - if empty or X then invalid
               % - need raw to determine if some rows empty before non-empty row is found
               % - if 'X' then something's gone wrong, else it'll be similar to 'X1'
               [~,~,raw] = xlsread(fullfile( logbookdir, filename), sheets{si}, 'B15:B100');
               ...
               [~,~,raw] = xlsread(fullfile( logbookdir, filename), sheets{si}, 'J15:J100');
               exp = raw( valid );
               ...
               if ~any( strcmpi( name, currnames ) )
                  ...
                  mouseexp{nummice}.scanrate = scanrate;
...
%% parameters set by seigfred in neuroSEE_batchProcess.m       
...
% dir = '/Volumes/Schultz_group_data/Crazy Eights/Ann/GCaMP6 imaging/2018.10.16/2P/';
%  
% % Enumerate the filenames of the TIFF videos to be processed
% video_names = {'20181016_10_15_57_2P/20181016_10_15_57_2P_XYT_1.tif',...
%     '20181016_10_15_57_2P/20181016_10_15_57_2P_XYT_2.tif'};
%  
...
% track_names = {'Track_2018-10-16-10-16-01/SavedTrack-2018-10-16-10-16-01.csv'
%     };
%  
