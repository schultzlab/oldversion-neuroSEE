...
%                       yyyymmdd_hh_mm_ss
%       display:    if =1, mean image with superimposed ROIs will be
%                       displayed. Fig is saved in filedir regardless.
%   OUTPUTS:
...
function [cell_tsG, cell_tsR, masks, fig] = ABLE( stack_g, mean_r, filedir, file, cellrad, maxcells, display )
   cprintf( 'Extracting ROIs with ABLE\n' )
   ...
   corr_g = crossCorr(stack_g);

   mean_R = mean_g ./ mean_r; 
   % Set the initial images 
   init_g = medfilt2( corr_g, szsmooth );
   R0     = mean_R - min(mean_R(:));
   R0     = R0 / max(R0(:));
   ...
   if retune_lambda
       ...
       [retuned_lambda, phi] = lambda_tune(phi_0, stack_g, mean_r, cellrad,...
                                           seg_opt, corr_g, R0);
   ...
   [masks, cell_tsG, nhbd_tsG, cell_tsR, nhbd_tsR] ...
