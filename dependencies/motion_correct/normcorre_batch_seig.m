function [M_final_r,M_final_g,shifts_g,template,options,col_shift] = normcorre_batch_seig(Y,X,options,template)
...
switch lower(options.output_type)
    case 'mat'
        M_final_r = zeros([sizY,T],data_type);
        M_final_g = zeros([sizY,T],data_type); %%%green channel
    case 'memmap'
        M_final_r = matfile(filename,'Writable',true);
        if nd == 2; M_final_r.Y(d1,d2,T) = zeros(1,data_type); end
        if nd == 3; M_final_r.Y(d1,d2,d3,T) = zeros(1,data_type); end
        M_final_r.Yr(d1*d2*d3,T) = zeros(1,data_type);        
    case {'hdf5','h5'}
    ...
    case {'tif','tiff'}
        M_final_r = ['motion corrected file has been saved as ', options.tiff_filename];
...
%                 end
            ...
            case 'mat'
                ...
                if nd == 3;
                    ...
                    Ytm2 = single(X(:,:,:,t:min(t+bin_width-1,T))); %%%green channel
                end
            case 'raw'
        ...
        if nd == 3; 
            ...
            Ytc2 = mat2cell(Ytm2,d1,d2,d3,ones(1,size(Ytm2,ndims(Ytm2)))); %%%green channel
        end
        Mf = cell(size(Ytc));
        Mf2 = cell(size(Ytc2)); %%%green channel
        lY = length(Ytc);
        ...
        parfor ii = 1:lY
            ...
            M_fin2 = cell(length(xx_us),length(yy_us),length(zz_us)); %zeros(size(Y_temp)); %%%green channel
                ...
                for j = 1:length(yy_s)           
                    for k = 1:length(zz_s)
                        ...
                        if all([length(xx_s),length(yy_s),length(zz_s)] == 1) && strcmpi(options.shifts_method,'fft');
                            ...
                            M_fin2{i,j,k} = shift_reconstruct(Yt2,shifts_temp(i,j,k,:),diff_temp(i,j,k),us_fac,Nr{i,j,k},Nc{i,j,k},Np{i,j,k},options.boundary,add_value);
                        end                                               
                    end
                end
            end            
            ...
            switch lower(options.shifts_method)
                case 'fft'
                        ...
                        for i = 1:length(xx_uf)
                            for j = 1:length(yy_uf)
                                for k = 1:length(zz_uf)
                                    ...
                                    I_temp2 = Yt2(extended_grid(1):extended_grid(2),extended_grid(3):extended_grid(4),extended_grid(5):extended_grid(6)); %%%green channel
                                    ...
                                    M_fin2{i,j,k} = shift_reconstruct(I_temp2,shifts_up(i,j,k,:),diff_up(i,j,k),us_fac,Nr{i,j,k},Nc{i,j,k},Np{i,j,k},options.boundary,add_value); %%%green channel
                    ...
                    if flag_interp    
                        ...
                        Mf2{ii} = cell2mat_ov_sum(M_fin2,xx_us,xx_uf,yy_us,yy_uf,zz_us,zz_uf,overlap_post,sizY,Bs) - add_value;
                    else            
                        Mf{ii} = cell2mat_ov(M_fin,xx_us,xx_uf,yy_us,yy_uf,zz_us,zz_uf,overlap_post,sizY) - add_value;
                        Mf2{ii} = cell2mat_ov(M_fin2,xx_us,xx_uf,yy_us,yy_uf,zz_us,zz_uf,overlap_post,sizY) - add_value;
                    end                    
        ...
        Mf = cell2mat(Mf);
        Mf2 = cell2mat(Mf2); %%%green channel
        ...
        switch lower(options.output_type)
            case 'mat'
                if nd == 2; M_final_r(:,:,t:min(t+bin_width-1,T)) = cast(Mf,data_type); end
                if nd == 3; 
                    M_final_r(:,:,:,t:min(t+bin_width-1,T)) = cast(Mf,data_type); 
                    M_final_g(:,:,:,t:min(t+bin_width-1,T)) = cast(Mf2,data_type);
                end
            case 'memmap'
                if rem_mem == options.mem_batch_size || t+lY-1 == T
                    if nd == 2; M_final_r.Y(:,:,t+lY-rem_mem:t+lY-1) = mem_buffer(:,:,1:rem_mem); end
                    if nd == 3; M_final_r.Y(:,:,:,t+lY-rem_mem:t+lY-1) = mem_buffer(:,:,:,1:rem_mem); end
                    M_final_r.Yr(:,t+lY-rem_mem:t+lY-1) = reshape(mem_buffer(1:d1*d2*d3*rem_mem),d1*d2*d3,rem_mem);
                end      
            case {'hdf5','h5'}
                if rem_mem == options.mem_batch_size || t+lY-1 == T
...
if memmap;
    M_final_r.shifts = shifts_g;
    M_final_r.template = template;
end
