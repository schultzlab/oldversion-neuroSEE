function [M_final_r, M_final_g,shifts,template,options,col_shift] = normcorre_seig(Y,X,options,template)
...
switch lower(options.output_type)
    case 'mat'
        ...
        M_final_g = zeros([sizY,T],data_type);
    case 'memmap'
    ...
    case {'tif','tiff'}
        M_final_r = options.tiff_filename;
    ...
    for t = 1:T
        switch filetype
            case 'tif'
                ...
                Yt2 = single(imread(X,'Index',t,'Info',tiffInfo)); %%%green channel
            case 'hdf5'
                Yt = single(h5read(Y,'/mov',[ones(1,nd),t],[sizY(1:nd),1]));
                Yt2 = single(h5read(X,'/mov',[ones(1,nd),t],[sizY(1:nd),1])); %%%green channel
            case 'mem'
                if nd == 2; 
                    Yt = single(Y.(var_name)(:,:,t)); 
                    Yt2 = single(X.(var_name2)(:,:,t)); 
                end
                if nd == 3; 
                    Yt = single(Y.(var_name)(:,:,:,t)); 
                    Yt2 = single(X.(var_name2)(:,:,:,t)); 
        ...
        M_fin2 = cell(length(xx_us),length(yy_us),length(zz_us)); %%%green channel
...
%             end
        ...
        switch lower(options.shifts_method)
            case 'fft'
                ...
                if flag_interp    
                    ...
                    Mf2 = cell2mat_ov_sum(M_fin2,xx_us,xx_uf,yy_us,yy_uf,zz_us,zz_uf,overlap_post,sizY,Bs) - add_value; %%%green channel
                else            
                    Mf = cell2mat_ov(M_fin,xx_us,xx_uf,yy_us,yy_uf,zz_us,zz_uf,overlap_post,sizY) - add_value;
                    Mf2 = cell2mat_ov(M_fin2,xx_us,xx_uf,yy_us,yy_uf,zz_us,zz_uf,overlap_post,sizY) - add_value; %%% green channel
                end                             
            ...
            otherwise
                ...
                if nd == 3                
                    ...
                    Mf2 = imwarp(Yt2,-cat(4,shifts_up(:,:,:,2),shifts_up(:,:,:,1),shifts_up(:,:,:,3)),options.shifts_method,'FillValues',fill_value); %%%green channel 
                else
                    ...
                    Mf2 = imwarp(Yt2,-cat(3,shifts_up(:,:,2),shifts_up(:,:,1)),options.shifts_method,'FillValues',fill_value); %%%green channel
                end    
        ...
        switch lower(options.output_type)
            case 'mat'
                if nd == 2; 
                    M_final_r(:,:,t) = cast(Mf,data_type);
                    M_final_g(:,:,t) = cast(Mf2,data_type); 
                end
                if nd == 3; 
                    M_final_r(:,:,:,t) = cast(Mf,data_type); 
                    M_final_g(:,:,:,t) = cast(Mf2,data_type); %%%green channel
                end
            case 'memmap'
                if rem_mem == options.mem_batch_size || t == T
                    if nd == 2; M_final_r.Y(:,:,t-rem_mem+1:t) = mem_buffer(:,:,1:rem_mem); end
                    if nd == 3; M_final_r.Y(:,:,:,t-rem_mem+1:t) = mem_buffer(:,:,:,1:rem_mem); end
                    M_final_r.Yr(:,t-rem_mem+1:t) = reshape(mem_buffer(1:d1*d2*d3*rem_mem),d1*d2*d3,rem_mem);
                end      
            case {'hdf5','h5'}
                if rem_mem == options.mem_batch_size || t == T
