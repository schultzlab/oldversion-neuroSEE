...
function[cellMasks, cellTimeSeriesG, nhbdTimeSeriesG, cellTimeSeriesR, nhbdTimeSeriesR, tempPhi] = ...
                               segment_cells_tune(phi_0, videoG, videoR, radius, options)
...
%%%added code by svp
video_reshapedR = reshape(videoR,[video_dim(1)*video_dim(2)*t_len,1,1]);
%%%
...
while ii <= size(phi,3)
   ...
   [timeSeriesG(ii,:), timeSeriesR(ii,:)] = extractTimeSeries(currentMask, mods, video_reshapedG, video_reshapedR, t_len);
...
while and(it <= maxIt, any(active))
    ...
    while ii <= size(phi,3)
       ...
       if active(ii)  
           ...
           [nG, nR]                = extractTimeSeries(nhbd_G, mods, video_reshapedG, video_reshapedR, t_len);
           ...
           if and(and(~remove, any(onlyThisMask(:))),...
               ...
               if and(it > noChangeNum, activity(ii,:) < delta)
                    active(ii)       = 0;
                    [timeSeriesG(ii,:), timeSeriesR(ii,:)] = extractTimeSeries(onlyThisMask, mods,video_reshapedG, video_reshapedR, t_len);
...
function [] = updatePlot(ii, videoG, phi, radius, nrows, ncols, corrIm, meanIm,...
    ...
    [n_tsG, n_tsR]    = extractTimeSeries(nhbd_G, mods, video_reshapedG, video_reshapedR, t_len);
