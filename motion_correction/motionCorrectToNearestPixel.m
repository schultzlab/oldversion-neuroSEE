% [stack_g, stack_r, fh, output] = motionCorrectToNearestPixel(red, green, doplot)
...
% Edit by Ann: lines 11, 12, 25-27, 68 

function [stack_g, stack_r, output, fh] = motionCorrectToNearestPixel(stack_g, stack_r, scale, doplot, Nimg_ave, filedir, file)
   ...
   redoT  = 400; % redo the first frames after template has converged
   ...
   if ~doplot, close( fh ); end

   fname_mat = [filedir file '_2P_mcorr_output.mat'];
   save(fname_mat,'-struct','output');
   
   prevstr = sprintf( 'Saving motion corrected tif images...\n' );
   cprintf('Text',prevstr);
   fname_tif_gr_mcorr = [filedir file '_2P_XYT_green_mcorr.tif'];
        writeTifStack( stack_g,fname_tif_gr_mcorr );
   fname_tif_red_mcorr = [filedir file '_2P_XYT_red_mcorr.tif'];
        writeTifStack( stack_r,fname_tif_red_mcorr );
   str = sprintf( 'Motion corrected tif images saved\n' );
